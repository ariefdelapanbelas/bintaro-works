...
import logoImg from "@/bintaro-works-logo.png";
import { cn } from "./primitives";

export const logoSrc = typeof logoImg === "string" ? logoImg : (logoImg as { src: string }).src;
  ...
  return (
    <span className={cn("inline-flex items-center gap-2.5 select-none", className)}>
      <img
        ...
        className={cn(
          imgSize,
          "rounded-lg object-cover shadow-sm ring-1 ring-black/15 dark:ring-white/15 shrink-0 transition-transform duration-200 group-hover:scale-105",
        )}
      />
      {!compact && (
